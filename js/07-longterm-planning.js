...
function exportCSV(){const h='Name,Spieleranzahl,Material,Abschnitt,Schwierigkeit,Beschreibung,Tags';const rows=exercises.map(e=>[e.name,e.players||'',e.material||'',e.section,e.difficulty||'',e.desc||'',(e.tags||[]).join('|')].map(v=>`"${String(v).replace(/"/g,'""')}"`).join(','));dl('svw_trainingsbuch.csv',[h,...rows].join('\n'),'text/csv');showToast('CSV exportiert');}
